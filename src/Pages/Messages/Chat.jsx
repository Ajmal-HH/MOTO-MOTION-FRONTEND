...
function Chat() {
    ...
    useEffect(() => {
        axios.get(`/messages/getreceiverdata`, { params: { receiverId } })
    ...
    useEffect(() => {
        if (socket) {
            ...
            socket.on("getUsers", (users) => setCurrentUsers(users));
    ...
    useEffect(() => {
        if (senderId && receiverId) {
            axios.get(`/messages/fetchchats/?id=${receiverId}&myId=${senderId}`)
                .then((response) => setMessageData(response.data?.messageData || []))
                    ...
                    <div className="relative flex items-center space-x-4">
                        ...
                        <div className="flex flex-col leading-tight" style={{ height: "100%" }}>
                            <div className="flex justify-end mb-2">
                                <Link
                                    to="/user-bookinglist"
                                    className="bg-teal-500 hover:bg-teal-700 rounded-md px-4 py-2 text-white text-sm font-medium"
                                >
                                    Back
                                </Link>
                            </div>
                ...
                >
                    {messageData?.map((message, index) => (
                        <div key={index} className="chat-message">
                            <div
                                className={`flex items-end ${message.senderId === receiverId ? "justify-start" : "justify-end"}`}
                            >
                                <div className="flex flex-col space-y-2 text-xs max-w-xs mx-2">
                                    <div>
                                        <span
                                            className={`px-4 py-2 rounded-lg inline-block ${message.senderId === receiverId ? "rounded-bl-none bg-gray-300 text-gray-600" : "rounded-br-none bg-blue-600 text-white"}`}
                                        >
                                            {message.message}
                                        </span>
                                    </div>
